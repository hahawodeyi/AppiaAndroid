...
- **M2 范围**：会话列表与房间聊天——列表三段分组/排序/搜索（搜索为占位入口，M5 接线）、未读徽标、左右滑快捷动作、连接横幅、下拉刷新、房间路由、消息渲染（系统消息/mention/草稿）、进房即读+停留标读、文本收发（echo 去重/状态徽标）、历史翻页、滚到底。
- **测试状态**：627 单测 + lint 全绿；Maestro 冒烟（`maestro/`）与模拟器真实会话走查通过。
